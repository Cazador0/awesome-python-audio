...
2. **Data** — [[redis]] hot path (cache, the [[task-queue]] Streams work queue, hot
   vectors), vector RAG, and RAPTOR Best-of-N evaluation against professional music
   community standards. Repo: `cazador0/ACE-Step-1.5`, `music_data_science/`.
3. **Memory & Context** — [[sqlite-store]] system of record with the embedded
   [[vector-store]] (durable vectors + RAPTOR persistence) plus this markdown vault:
...
`project/PROMPT_TO_STEM.md` and operationalized in [[prompt-translation]] (optionally
sharpened by the [[blueprint-refiner]]), [[stem-regeneration]], and
